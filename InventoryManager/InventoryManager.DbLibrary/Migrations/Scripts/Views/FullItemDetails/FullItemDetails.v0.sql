...
AS 
SELECT item.Id, item.[Name] ItemName, item.[Description] ItemDescription , item.IsActive, item.IsDeleted, item.Notes, 
	item.CurrentOrFinalPrice , item.IsOnSale, item.PurchasedDate, item.PurchasePrice, item.Quantity, 
...
LEFT JOIN ItemGenges ig on item.id = ig.ItemId 
